...
pub enum BaudRate {
    ...
    Baud9600,
    /** 14,400 baud. */
    #[cfg(any(target_os = "freebsd", target_os = "dragonfly", target_os = "macos", target_os = "netbsd", target_os = "openbsd", windows))]
    Baud14400,
    /** 19,200 baud. */
    Baud19200,
    /** 28,800 baud. */
...
mod tests {
    #[test]
    fn test_available_rates() {
        use ::BaudRate;
        #[cfg(unix)]
        use posix::available_baud_rates;
        #[cfg(windows)]
        use windows::available_baud_rates;
        for rate in available_baud_rates().iter() {
            BaudRate::from(*rate);
        }
    }
}
